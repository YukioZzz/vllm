...
class KVCacheStoreSendingThread(KVTransferThread):
    ...
    def _maybe_offload_partial_tail(self, req_meta: ReqMeta) -> bool:
        ...
        for g_idx, db in enumerate(self.token_databases):
            ...
            for block_idx in range(
                ...
                spec = self.coord.kv_cache_groups[g_idx].kv_cache_spec
    ...
    def _handle_request(self, req_meta: ReqMeta):
        ...
        try:
            ...
            kv_event_parent_hashes: list[Any] = []
            if self.enable_kv_event:
                prev_key_per_group: dict[int, Any] = {}
                new_block_hashes = [
                    maybe_convert_block_hash(bh) for bh in kv_event_block_hashes
                ]
                for block_hash, g_idx in zip(
                    new_block_hashes, group_indices, strict=True
                ):
                    kv_event_parent_hashes.append(prev_key_per_group.get(g_idx))
                    prev_key_per_group[g_idx] = block_hash
            ...
            if len(missing_indices) != len(keys):
                ...
                if self.enable_kv_event:
                    ...
                    kv_event_parent_hashes = [
                        kv_event_parent_hashes[i] for i in missing_indices
                    ]
            ...
            if self.enable_kv_event:
                for idx, (s, e, g_idx) in enumerate(
                    ...
                    stored_event = BlockStored(
                        ...
                        parent_block_hash=kv_event_parent_hashes[idx],
                        token_ids=token_ids,
...
class MooncakeStoreWorker:
    ...
    def __init__(
        ...
        # Match the core cache manager's effective block geometry. DCP shards
        # full-attention KV across ranks, so one externally addressable block
        # spans ``dcp_size`` physical blocks. Mamba state remains replicated and
        # must not be scaled.
