...
class MooncakeStoreConnector(KVConnectorBase_V1, SupportsHMA):
    ...
    def _validate_kv_cache_config(
        ...
        if is_hybrid and pcp > 1:
            unsupported.append(
                f"PCP > 1 (pcp={pcp}) with hybrid attention"
            )
        if is_hybrid and dcp > 1:
            for g_idx, g in enumerate(kv_cache_config.kv_cache_groups):
                spec = g.kv_cache_spec
                if isinstance(spec, UniformTypeKVCacheSpecs):
                    spec = next(iter(spec.kv_cache_specs.values()))
                if not isinstance(spec, (FullAttentionSpec, MambaSpec)):
                    unsupported.append(
                        f"group {g_idx}: {type(spec).__name__} with DCP > 1"
                    )
        if unsupported:
            raise ValueError(
