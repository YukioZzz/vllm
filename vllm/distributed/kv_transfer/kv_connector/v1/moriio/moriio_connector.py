...
class MoRIIOConnectorScheduler:
            ...
            if self.mode == MoRIIOMode.READ:
                if remote_block_ids := params.get("remote_block_ids"):
                    ...
                    if "remote_engine_id" in params:
                        ...
                        remote_attn, _ = split_attn_mamba_block_ids(remote_block_ids)
    ...
    def split_block_groups(
        ...
        into the recurrent-state slot list. For a pure-attention
...
class MoRIIOConnectorWorker:
    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor]):
        ...
        if self._has_mamba:
            ...
            if self.mode == MoRIIOMode.WRITE:
                # Only the READ path moves the recurrent state. In WRITE mode
                # wait_for_save would still sweep the KDA layers into the write
                # queue, where the plan builder has no conv/ssm case: the task
                # raises, the writer marks it done, and the decoder is told the
                # transfer succeeded while the state never moved. Refuse instead
                # of serving a zero recurrent state. WRITE support is a
                # follow-up.
                raise MoRIIOError(
