        ...
        if params is not None and params.get("do_remote_prefill"):
            if self.mode == MoRIIOMode.READ:
                ...
                if _should_notify:
                    ...
                    # Attention may need nothing pushed (cache hit / len-1
                    # accounting), but the per-request KDA recurrent state is
                    # not prefix-cacheable and must still be pushed. With attn
                    # tokens, notify [attn, mamba]; otherwise notify just the
                    # mamba slot so the producer still writes the recurrent
                    # state into the decoder's blocks.
                    if num_external_tokens > 0:
...
class MoRIIOConnectorWorker:
    ...
    def _get_built_session(self, remote_engine_id):
        ...
        if getattr(self, "_draft_only_layers", None) is None:
            # Speculative-decode (dspark) draft-model layers exist only on the
            # decoder; the prefiller never registered them, so there is no
            # remote KV to pull. Record them once and skip everywhere (session
            # build, region-index map, read loop) so the flat session indexing
            # stays consistent. Their state is recomputed locally on decode.
    ...
    def _region_session_indices(self, layer_name: str) -> list[int]:
        ...
        if self._region_session_index is None:
            ...
            idx = 0
            draft_only = getattr(self, "_draft_only_layers", None) or set()
    ...
    def _read_blocks(
        ...
        _draft_only = getattr(self, "_draft_only_layers", None) or set()
        for layer_name in self.layer_name_to_local_kv_cache_metadata:
            if layer_name in _draft_only:
                # Draft-model layer (spec decode) not present on the prefiller;
                # no remote KV to pull, its state is recomputed locally.
                continue
