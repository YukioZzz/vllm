                    ...
                    if "remote_engine_id" in params:
                        ...
                        if num_external_tokens > 0:
                            ...
                            if self.dcp_size > 1:
                                ...
                                pass
                            elif len(local_attn_groups) == len(remote_attn_groups):
...
class MoRIIOConnectorWorker:
    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor]):
        ...
        shared_backing = all(
            isinstance(t, torch.Tensor) for t in kv_caches.values()
        ...
        **kwargs,
        ...
        for req_id, meta in metadata.reqs_to_save.items():
            ...
            num_prompt_tokens = int(
                kwargs.get("num_prompt_tokens", meta.num_prompt_tokens) or 0
            )
            self._write_blocks_for_req(
                req_id, meta, layer_name, kv_layer, num_prompt_tokens
            )
        ...
        while True:
            if (
                ...
                num_prompt_tokens = int(
                    kwargs.get("num_prompt_tokens", ready_meta.num_prompt_tokens) or 0
                )
                self._write_blocks_for_req(
                    ready_req_id,
                    ready_meta,
    ...
    def _dcp_pair_attention_blocks(
        ...
        if granularity == DCP_GRANULARITY_TOKEN:
            ...
            live_tokens = (
                min(num_prompt_tokens or 0, len(prefill_ids) * block_size) or None
            )
    ...
    def _compute_block_transfer_offsets(
        ...
        if dcp_granularity == DCP_GRANULARITY_TOKEN:
            ...
            local, remote, sizes = compute_dcp_token_transfer_offsets(
                layer_name=layer_name,
        ...
        # Both granularities produce offsets relative to the layer's own cache,
        # so both have to be rebased onto the shared memory region.
