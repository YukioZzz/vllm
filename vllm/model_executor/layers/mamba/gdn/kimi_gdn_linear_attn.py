    ...
    def _forward(
        ...
        if mixed_qkv_ns is not None:
            ...
            if m.num_prefills > 0:
                ...
                assert non_spec_state_indices_tensor is not None
                # Under spec decode the metadata builder can hand back a strided
                # (1-D but stride!=1) non-spec index view; the packed KDA decode
                # kernel requires a contiguous 1-D state_indices, so compact it.
