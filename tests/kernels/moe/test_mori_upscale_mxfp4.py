...
"""Bit-exact correctness for the MoRI MXFP4 upscale Triton kernel.

The kernel runs after MoRI dispatches an MXFP4 (``float4_e2m1fn_x2``)
activation payload + per-32 ``float8_e8m0fnu`` scale; downstream
fused-MoE expert kernels expect unpacked BF16/FP16 hidden states, so the
dispatcher upcasts via ``upscale_mxfp4``.

We feed the kernel codes generated by a Python MXFP4 reference quantizer
and assert the kernel's decode is bit-identical to the reference decode
(the quantizer is shared so any rounding error is the same on both
sides; the kernel's only job is to undo the FP4 encoding).
"""
...
def _mxfp4_quant_reference(x: torch.Tensor, group: int = 32):
    ...
    ``payload`` and ``scale`` use the same on-wire layout the kernel
    expects; ``dequant_ref`` is the exact reference de-quantization that
...
def test_upscale_mxfp4_zero_recv_returns_shaped_buffer():
    """When the dispatcher reports zero received tokens, the kernel must
    return a properly-shaped output without touching memory (and without
    raising)."""
    device = "cuda"
    M, N = 8, 64
