...
def test_dcp_verify_rows_sum_to_the_global_window_across_ranks():
    ...
    expected = [
        int(seq_len) - qlen + t + 1 for seq_len in seq_lens.tolist() for t in range(qlen)
    ]
...
def test_dcp_fp8_verify_build_prefers_gluon(monkeypatch):
    qlen = 4
    monkeypatch.setattr(
        rocm_aiter_mla,
...
def test_dcp_fp8_verify_build_uses_segmented_without_gluon(monkeypatch):
    qlen = 4
    monkeypatch.setattr(rocm_aiter_mla, "_gluon_mla_decode_supported", lambda: True)
    monkeypatch.setattr(
