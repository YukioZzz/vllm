...
def test_dcp_verify_row_view_excludes_in_hand_window():
    ...
    pages, row_lens, qo_indptr, max_kv_len = (
...
def test_dcp_verify_row_view_uses_static_graph_bound():
    ...
    builder._dcp_verify_block_table = torch.zeros(
        (qlen, 24), dtype=torch.int32
    )
...
def test_verify_partial_attention_merge():
    ...
    out, lse = _lse_combine_natural(
        out_a, lse_a, out_b, lse_b, torch.float32
    )
    normalizer = torch.logaddexp(lse_a, lse_b)
    expected = (
        out_a * torch.exp(lse_a - normalizer).unsqueeze(-1)
        + out_b * torch.exp(lse_b - normalizer).unsqueeze(-1)
    )
