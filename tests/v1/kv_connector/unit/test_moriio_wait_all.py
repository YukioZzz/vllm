...
Covers the two behaviours the READ barrier depends on:
  ...
  * waiting_for_transfer_complete blocks until terminal and raises on failure,
    identically on a mori that exposes the batched wait (ROCm/mori#341) and on
    one that does not.
"""
...
class Mori341Engine:
    """IOEngine with the WaitAll semantics of ROCm/mori#341."""
...
class LegacyEngine:
    """Pre-#341 build: no wait_all attribute at all."""
