...
def _owner_of_token(
    ...
    return (
        owner,
        virtual_block_index + local_offset // block_size,
        (local_offset % block_size),
    )


def _owner_of_token_split(
    pos: int,
    ...
    """Same port with kernel block splitting left in.

    An attention kernel that wants smaller pages than the allocator hands out
    makes ``BLOCKS_PER_KV_BLOCK`` exceed 1, and the block table is expanded so
    that manager block ``b`` occupies kernel blocks ``b * BLOCKS_PER_KV_BLOCK``
    through ``(b + 1) * BLOCKS_PER_KV_BLOCK - 1``. Returns
    ``(owner_dcp_rank, kernel_block_index, slot_within_kernel_block)``.
    """
    blocks_per_kv_block = manager_block_size // kernel_block_size
    virtual_block_size = manager_block_size * dcp_size
    virtual_block_index = pos // virtual_block_size
    virtual_block_offset = pos % virtual_block_size
    owner = (virtual_block_offset // interleave) % dcp_size
    local_offset = (virtual_block_offset // (dcp_size * interleave)) * interleave + (
    ...
    kernel_block_index = (
        virtual_block_index * blocks_per_kv_block + local_offset // kernel_block_size
    )
    return owner, kernel_block_index, local_offset % kernel_block_size
...
@pytest.mark.parametrize("kernel_block_size", [16, 32, 64])
@pytest.mark.parametrize("dcp_size", [2, 4, 8])
@pytest.mark.parametrize("block_regime", [True, False])
def test_kernel_block_splitting_only_renames_the_slot(
    kernel_block_size, dcp_size, block_regime
):
    """Why the relayout may reason in allocation blocks and ignore splitting.

    The pairing works in allocation-block space, because that is what
    ``KVCacheSpec.block_size`` and the transfer geometry report. Kimi-K3 pages
    are large (768 / 1536 tokens), so an attention kernel asking for smaller
    pages would put ``BLOCKS_PER_KV_BLOCK > 1`` between the two. That is only a
    change of variables: splitting keeps the owning rank identical and merely
    renumbers a position inside an allocation block as (kernel block, slot).
    If that ever stops holding, the relayout has to learn about kernel blocks.
    """
    manager_block_size = 64
    blocks_per_kv_block = manager_block_size // kernel_block_size
    interleave = manager_block_size if block_regime else 1

    for pos in range(4 * manager_block_size * dcp_size):
        owner, kernel_block, kernel_slot = _owner_of_token_split(
            pos, manager_block_size, kernel_block_size, dcp_size, interleave
        )
        manager_owner, manager_block, manager_slot = _owner_of_token(
            pos, manager_block_size, dcp_size, interleave
        )
        assert owner == manager_owner
        assert kernel_block // blocks_per_kv_block == manager_block
        offset_in_manager_block = (
            kernel_block % blocks_per_kv_block
        ) * kernel_block_size + kernel_slot
        assert offset_in_manager_block == manager_slot
...
def test_token_pairing_respects_num_tokens():
    ...
    positions = [(s // block_size - 10) * block_size + s % block_size for s in p_slots]
...
def test_token_pairing_respects_absolute_position():
    ...
    positions = [
        (s // block_size - 70) * block_size + s % block_size + 12 for s in p_slots
    ]
    ...
    live = sum(
        len(
            build_dcp_token_pairing(
                prefill, decode, block_size, dcp_size, r, num_tokens=num_tokens
            )[0]
        )
